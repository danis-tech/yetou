...
export default function PhotoDetailPage({ params }: { params: Promise<{ index: string }> }) {
  ...
  const handleBuy = useCallback(async () => {
    ...
    const ok = await checkout({
      mediaId: media.id,
      buyItem: { name: media.title, price: String(media.price), format: media.license_type, img: media.preview_url || media.thumbnail_url, _type: "photo", mediaId: media.id },
    ...
    <div style={{ background: "var(--paper)", minHeight: "100vh" }}>
      ...
      <div className="detail-hero">
        <div className="detail-hero-inner">
          ...
          {!purchased && <div style={{ position: "absolute", top: "12px", right: "12px", zIndex: 20, background: "rgba(20,35,58,0.78)", color: "#fff", fontSize: "12px", fontWeight: 600, padding: "4px 10px", borderRadius: "4px" }}><i className="ti ti-eye" style={{ marginRight: "5px" }}></i>Aperçu filigrané</div>}
          {!purchased && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none", zIndex: 5 }}><span style={{ fontFamily: "var(--font)", fontSize: "3rem", fontWeight: 700, color: "rgba(255,255,255,0.1)", letterSpacing: "0.2em", transform: "rotate(-15deg)" }}>Pixia</span></div>}
          ...
          {purchased && <div style={{ position: "absolute", top: "12px", left: "12px", zIndex: 20, background: "rgba(47,125,79,0.9)", color: "#fff", fontSize: "12px", fontWeight: 700, padding: "5px 14px", borderRadius: "8px" }}><i className="ti ti-circle-check" style={{ marginRight: "5px" }}></i>Achetée</div>}
          <div style={{ position: "absolute", top: "12px", left: purchased ? "auto" : "12px", right: purchased ? "12px" : "auto", background: display.is4k ? "var(--laterite)" : "rgba(20,35,58,0.78)", backdropFilter: "blur(6px)", color: "#fff", fontSize: "12px", fontWeight: 700, padding: "4px 12px", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.08)" }}>{media.quality_display}</div>
        </div>
      </div>

      <div className="detail-body">
        <div>
          <h1 style={{ fontFamily: "var(--font)", fontSize: "24px", fontWeight: 700, color: "var(--ink)", marginBottom: "8px" }}>{display.title}</h1>
          ...
          <p style={{ fontSize: "14px", color: "var(--ink-2)", lineHeight: 1.6, marginBottom: "24px" }}>{display.desc}</p>
          ...
          {(display.lens || display.focal || display.aperture || display.iso || display.shutter) && (
            <div className="detail-info-block">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "8px", fontSize: "13px", color: "var(--ink-2)" }}>
                {display.lens && <div>Objectif : <span style={{ color: "var(--ink)" }}>{display.lens}</span></div>}
                {display.focal && <div>Focale : <span style={{ color: "var(--ink)" }}>{display.focal}</span></div>}
                {display.aperture && <div>Ouverture : <span style={{ color: "var(--ink)" }}>{display.aperture}</span></div>}
                {display.iso && <div>ISO : <span style={{ color: "var(--ink)" }}>{display.iso}</span></div>}
                {display.shutter && <div>Obturation : <span style={{ color: "var(--ink)" }}>{display.shutter}</span></div>}
                {display.altitude && <div>Altitude : <span style={{ color: "var(--ink)" }}>{display.altitude} m</span></div>}
              </div>
            </div>
          )}
          {(display.tags || display.season || display.weather) && (
            <div className="detail-info-block">
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {display.tags && (display.tags).split(",").map((t, i) => <span key={i} style={{ background: "rgba(47,111,115,0.1)", color: "var(--river)", fontSize: "11px", padding: "4px 10px", borderRadius: "8px", border: "1px solid rgba(47,111,115,0.2)" }}>{t.trim()}</span>)}
                {display.season && <span style={{ background: "var(--paper)", color: "var(--ink-2)", fontSize: "11px", padding: "4px 10px", borderRadius: "8px", border: "1px solid var(--contour)" }}>{display.season}</span>}
                {display.weather && <span style={{ background: "var(--paper)", color: "var(--ink-2)", fontSize: "11px", padding: "4px 10px", borderRadius: "8px", border: "1px solid var(--contour)" }}>{display.weather}</span>}
              </div>
            </div>
          )}
        </div>
        <div style={{ position: "relative" }}>
          <div className="detail-sidebar-sticky">
            <div style={{ marginBottom: "20px" }}>
              <div style={{ fontFamily: "var(--font)", fontSize: "28px", fontWeight: 700, color: "var(--ink)" }}>{display.priceStr}</div>
              <div style={{ fontSize: "12px", color: "var(--ink-2)", marginTop: "4px" }}>Paiement sécurisé</div>
            </div>
            ...
            {purchased ? (
              <div style={{ textAlign: "center", padding: "16px", background: "rgba(47,125,79,0.1)", border: "1px solid rgba(47,125,79,0.3)", borderRadius: "10px" }}>
                ...
                <a href="/dashboard?tab=downloads" className="btn-primary" style={{ display: "inline-flex", alignItems: "center", gap: "8px", textDecoration: "none", padding: "10px 20px" }}><i className="ti ti-download"></i> Télécharger la photo</a>
              </div>
            ) : (
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", marginTop: "12px", fontSize: "11px", color: "var(--ink-2)" }}>
              <i className="ti ti-shield-check" style={{ color: "var(--ok)" }}></i> Paiement 100% sécurisé
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="detail-related-grid">
          {related.map((m) => (
            <div key={m.id} onClick={() => router.push(`/photo/${m.id}`)}
              style={{ background: "var(--card)", border: "1px solid var(--contour)", borderRadius: "10px", overflow: "hidden", cursor: "pointer", transition: "all .2s" }}
              ...
              <div style={{ padding: "10px 14px" }}>
                <div style={{ fontFamily: "var(--font)", fontSize: "12px", fontWeight: 600, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.title}</div>
                <div style={{ fontSize: "11px", color: "var(--ink-2)", marginTop: "4px" }}>{m.quality_display} · {m.price.toLocaleString("fr-FR")} FCFA</div>
              </div>
            </div>
          ))}
        </div>
...
function NotFound({ router }: { router: ReturnType<typeof useRouter> }) {
  return <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "16px" }}>
    <i className="ti ti-photo-off" style={{ fontSize: "48px", color: "var(--ink-2)" }}></i>
    <p style={{ color: "var(--ink)" }}>Photo introuvable.</p>
